...
noexcept(cv::is_conversion_noexcept_v<C>) -> point<T1, C>& {
    lhs.x += rhs.x;
    lhs.y += rhs.y;
    return lhs;
}
...
noexcept(cv::is_conversion_noexcept_v<C>) -> point<T, C>& {
    lhs.x += rhs;
    lhs.y += rhs;
    return lhs;
}
...
struct compound_op<T, U>
{ using type = T (&)(T, U); };

template<typename T1, typename T2, typename C>
struct compound_op<point<T1, C>, point<T2, C>>
    : compound_op<point<std::common_type_t<T1, T2>, C>&, point<T2, C> const&> {};

template<typename U, typename T, typename C>
struct compound_op<U, point<T, C>>
    : compound_op<point<U, C>, point<T, C>> {};

template<typename T, typename C, typename U>
struct compound_op<point<T, C>, U>
    : compound_op<point<std::common_type_t<T, U>, C>&, U> {};

template<typename T, typename C>
struct compound_op<point<T, C>>
    : compound_op<T, point<T, C>> {};

template<typename... Ts>
using compound_op_t = typename compound_op<Ts...>::type;
...
constexpr auto apply(
    ...
    point<T, C> const& lhs,
    U const& rhs
) noexcept(cv::is_conversion_noexcept_v<C>) -> R {
    auto temp = R{lhs};
    operation(temp, rhs);
    return temp;
}
...
constexpr auto dispatch(
    compound_op_t<T1, point<T2, C>> operation,
...
constexpr auto dispatch(
    compound_op_t<T2, point<T1, C>> operation,
    point<T1, C> const& lhs,
    point<T2, C> const& rhs
) noexcept(cv::is_conversion_noexcept_v<C>)
...
template<typename T, typename U, typename C,
    typename = ts::require<std::is_arithmetic<U>>>
constexpr auto dispatch(
    compound_op_t<point<T, C>, U> operation,
    point<T, C> const& lhs,
    U rhs
) noexcept(cv::is_conversion_noexcept_v<C>)
{ return (apply)(operation, lhs, rhs); }
...
noexcept(cv::is_conversion_noexcept_v<C>) -> point<ts::promote<T>, C>
{ return src; }
...
template<typename T, typename C>
constexpr auto dispatch(
    compound_op_t<point<T, C>> operation,
    point<T, C>&& lhs,
    point<T, C>&& rhs
) noexcept(cv::is_conversion_noexcept_v<C>) -> point<T, C>&&
{ return (apply)(operation, std::move(lhs), rhs); }
...
noexcept(cv::is_conversion_noexcept_both_v<T1, T2>)
